...
public class PopMenuActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    private void doAnimateOpen(final View view, int index, int total, int radius) {
        if (view.getVisibility() != View.VISIBLE) {
            view.setVisibility(View.VISIBLE);
        }
        double degree = Math.toRadians(90)/(total - 1) * index;//得到角度对应的弧度，因为分成了total-1份
        int translationX = -(int) (radius * Math.sin(degree));//半径乘以Math.sin(degree)得到x轴方向上的偏移
    ...
    private void doAnimateClose(final View view, int index, int total,
        ...
        set.playTogether(
                ...
                ObjectAnimator.ofFloat(view, "scaleY", 1f, 0.1f),//不能写成0f，缩小到0以后不会实际改变控件位置，导致点击menu点不到
